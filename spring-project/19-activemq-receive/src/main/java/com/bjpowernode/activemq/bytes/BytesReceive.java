package com.bjpowernode.activemq.bytes;
...
public class BytesReceive {
    ...
    private static void receive() {
        ...
        try {
            ...
            Destination destination=session.createQueue("myByte");
            ...
            if(message instanceof BytesMessage){
                //注意：读取数据时，要和写入数据时的顺序保持一致，否则数据就会乱，也有可能会导致异常
                System.out.println("年龄="+((BytesMessage) message).readInt());
                System.out.println("姓名="+((BytesMessage) message).readUTF());
